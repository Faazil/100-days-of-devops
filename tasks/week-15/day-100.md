...
The Nautilus DevOps team needs to setting up an EC2 instance for their application. To guarantee the application runs efficiently, also requiring create a CloudWatch alarm for monitoring the instance's CPU utilization. The alarm triggers when the CPU utilization goes above 90% for one continuous 5-minute period. For notifications, use the SNS topic named nautilus-sns-topic, that exists.
...
2. Create CloudWatch Alarm: Create a CloudWatch alarm named `nautilus-alarm` with the following specifications:
    ...
    - Threshold: `>= 90%` for `1` continuous `5-minute` period
...
**Step 2:**
```bash
> There is a complete terraform file: [main.tf](../files/terraform_cloudwatch_alarm_on_ec2_instance_100.tf) with vpc, subnet, igw, route table, security group, ec2 key pair, sns topic and cloudwatch alarm. You may give it a look.
...
- **← Previous**: [Day 99 - Attach IAM Policy for DynamoDB Access Using Terraform](./day-99.md)
